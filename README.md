## 學生資訊 - 姓名：闕愷頡 - 學號：B3230622 - 課程：3468 演算法 1142 
## 實驗環境 - 平台：Google Colab T4 / WSL2 / 純 Linux（擇一） - vLLM 版本：0.7.2 - 模型：Qwen/Qwen2.5-0.5B-Instruct - prompts 數：100 - max_tokens：64 
## 結果摘要 
========== 比較摘要 ==========
...
## 結論 
這次實驗結果超有感，開啟 Prefix Caching 後，系統總耗時直接從 4.86s 掉到 1.60s，吞吐量（req/s 和 tok/s）瞬間飆升了 3.05 倍，完全印證了投影片上寫的優化幅度

這背後的原理其實就是我們在 CLRS §11.4 學到的雜湊表（Hash Table）應用。當關閉快取時，100 個請求都要對那段 ~800 tokens 的長前綴重複做 Prefill 計算，根本是純浪費算力。而 vLLM 的優化方式，是把這段共用前綴的 Token IDs 序列轉成 block_hash，並在記憶體維護一個 `Dict[block_hash, PhysicalBlock]。

後續 99 個請求進來時，只需要花 $O(1)$ 的時間複雜度去 Look-up 雜湊表，命中（Cache Hit）後就能直接拿現成的 KV Block 來用，前綴計算量直接歸零。因為 LLM 的輸入序列很難被刻意構造出雜湊碰撞（Hash Collision）來進行惡意攻擊，所以不需要用到複雜的 Universal Hashing，用內建的 Deterministic Hashing 就能在平均狀況下穩穩達到 $O(1)$ 查找，完美用空間換取超大的時間回報
